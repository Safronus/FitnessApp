...
class FitnessTrackerApp(QMainWindow):
    ...
    def _persist_bmi_plan_settings(self, *_) -> None:
        ...
        # (4.4.3) Vždy po změně parametrů plánu přepočítat a překreslit týdenní rozpis
        try:
            self.recompute_bmi_plan()
            self.apply_weekly_plan_gradient()
        except Exception:
    ...
    def _persist_plan_start_date(self, qdate) -> None:
        ...
        # (4.4.3) Vždy po změně začátku plánu přepočítat a překreslit týdenní rozpis
        try:
            self.recompute_bmi_plan()
            self.apply_weekly_plan_gradient()
        except Exception:
    ...
    def recompute_bmi_weekly_breakdown(
        ...
        for i in range(7):
            ...
            col = (
                ...
                _lerp(red[2], yellow[2], t),
            )
            ax.axhspan(
                y0,
                y1,
                facecolor=_rgb_to_hex(col),
                alpha=0.10,
                zorder=0,
            )
